...
export async function returnBook(borrowId: string) {
  ...
  try {
    // Get borrow details
    const { data: borrow } = await supabase.from("borrows").select("book_id, status").eq("id", borrowId).single()
    ...
    const { data: book } = await supabase.from("books").select("available_copies").eq("id", borrow.book_id).single()
