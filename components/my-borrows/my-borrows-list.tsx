...
export function MyBorrowsList({ borrows, showReturnButton = true }: { borrows: Borrow[]; showReturnButton?: boolean }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {showReturnButton ? "You don't have any active loans" : "No return history yet"}
        </p>
        {showReturnButton && (
  ...
  return (
    <div className="grid gap-4">
      {borrows.map((borrow) => {
        const isOverdue = isPast(new Date(borrow.due_date)) && !borrow.return_date
          ...
          >
            <div className="flex gap-4">
              ...
              <div className="flex flex-col gap-1">
                <Link href={`/books/${borrow.book.id}`} className="font-semibold hover:underline">
                  {borrow.book.title}
                </Link>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1">
                  ...
                  {borrow.return_date && <Badge variant="secondary">Returned</Badge>}
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-muted-foreground">
                  ...
                  <span className={isOverdue ? "font-medium text-destructive" : ""}>
                    {format(new Date(borrow.return_date || borrow.due_date), "MMM d, yyyy")}
                  </span>
                </span>
              </div>
