...
export default async function BooksPage({ searchParams }: PageProps) {
  ...
  const { data: categoriesData } = await supabase.from("books").select("category").not("category", "is", null)

  const categories = Array.from(new Set(categoriesData?.map((b) => b.category).filter(Boolean) || []))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">Browse Collection</h1>
          <p className="mt-1 text-muted-foreground">Discover and borrow from our extensive library</p>
        </div>
