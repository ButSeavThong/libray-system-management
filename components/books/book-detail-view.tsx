  ...
  const handleBorrow = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          title: "Success!",
          description: `You have successfully borrowed "${book.title}". Due date: ${new Date(result.dueDate!).toLocaleDateString()}`,
        })
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[400px_1fr]">
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-6">
            <div className="flex flex-col gap-3">
              <Button onClick={handleBorrow} disabled={!isAvailable || isBorrowing} size="lg" className="w-full">
                {isBorrowing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                {hasActiveBorrow ? "Already Borrowed" : book.available_copies === 0 ? "Out of Stock" : "Borrow Book"}
              </Button>
      ...
      <div className="flex flex-col gap-6">
        <div>
          <h1 className="text-balance text-4xl font-bold tracking-tight">{book.title}</h1>
          ...
          <CardContent className="grid gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">14 days from borrow date</p>
              </div>
            </div>
          </CardContent>
        </Card>
